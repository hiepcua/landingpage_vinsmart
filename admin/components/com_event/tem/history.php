...
$res_Events = SysGetList('tbl_event', array(), 'AND id='.$GetID);
$res_Cons = SysGetList('tbl_event_history', array(), 'AND event_id='.$GetID.' ORDER BY cdate DESC');
if(count($res_Cons) <= 0){
		...
		<div class="row mb-2">
			<div class="col-md-12">
				<ol class="breadcrumb">
					...
					<li class="breadcrumb-item"><a href="<?php echo ROOTHOST.COMS.'/edit/'.$GetID;?>"><?php echo $res_Events[0]['title'];?></a></li>
					<li class="breadcrumb-item active">Lịch sử hoạt động khoa học</li>
				</ol>
			</div><!-- /.col -->
		</div><!-- /.row -->
	</div><!-- /.container-fluid -->
			...
			<div class='table-responsive'>
				<table class="table table-bordered">
					<thead>                  
						<tr>
							<th>Tác giả</th>
							<th>Tiêu đề hoạt động khoa học (hiện tại)</th>
							<th>Hành động</th>
							<th>Thời gian</th>
							<th>Công cụ</th>
						</tr>
...
<script type="text/javascript">
	function view_history(e){
		...
		var _url="<?php echo ROOTHOST;?>ajaxs/event/view_history.php";
		var _data = {
			"id": _id,
		}
		$.post(_url, _data, function(res){
			$('#popup_modal .modal-dialog').addClass('modal-lg');
			$('#popup_modal .modal-title').html('Lịch sử hoạt động khoa học');
