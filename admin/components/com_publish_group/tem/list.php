<?php
define('OBJ_PAGE','publish_group');
$strWhere=""; $limit=''; $flg_search = 0;
...
if(count(array_intersect($arr_permission, array_keys(PERMISSION_PUBLISH_GROUP))) <= 0 && $isAdmin!=1){
...
<div class="content-header">
	<div class="container-fluid">
		<div class="row mb-2">
			<div class="col-sm-6">
				<h1 class="m-0 text-dark">Danh sách loại xuất bản</h1>
			</div><!-- /.col -->
			<div class="col-sm-6">
				<ol class="breadcrumb float-sm-right">
					<li class="breadcrumb-item"><a href="<?php echo ROOTHOST;?>">Bảng điều khiển</a></li>
					<li class="breadcrumb-item active">Danh sách loại xuất bản</li>
				</ol>
			</div><!-- /.col -->
		</div><!-- /.row -->
	</div><!-- /.container-fluid -->
		...
		<div class="row widget-frm-search form-group">
			<div class="col-md-6">
				<form id="frm_search" method="get" action="<?php echo ROOTHOST.COMS;?>">
					<div class="frm-search-box form-inline pull-left">
						<input type='text' id='txt_title' name='q' class='form-control' value="<?php echo $get_q?>" placeholder="Tên loại xuất bản..." />
						&nbsp&nbsp&nbsp
			...
			<div class="col-md-6">
				<div class="pull-right">
					...
					<?php if(in_array(9001, $arr_permission)) { ?>
			...
			<div class='table-responsive'>
				<table class="table table-bordered">
					<thead>                  
						<tr>
							<th>Xóa</th>
							<th>Tên loại xuất bản</th>
							<th>Nhóm xuất bản cha</th>
							<th>Mô tả</th>
...
<script type="text/javascript">
	...
	function add(){
		var _url="<?php echo ROOTHOST;?>ajaxs/publish_group/form_add.php";
		$.get(_url, function(req){
			$('#popup_modal .modal-dialog').addClass('modal-xl');
			$('#popup_modal .modal-title').text('Thêm mới loại xuất bản');
	...
	function getTable(strwhere, search, limit){
		var _url="<?php echo ROOTHOST;?>ajaxs/publish_group/get_table.php";
		var _data={
			"strwhere": strwhere,
			"search": search,
			"limit": limit,
	...
	function edit(id){
		if(parseInt(id)!==0){
			var _url="<?php echo ROOTHOST;?>ajaxs/publish_group/form_edit.php";
			$.get(_url, {"id": id}, function(req){
				$('#popup_modal .modal-dialog').addClass('modal-xl');
				$('#popup_modal .modal-title').text('Cập nhật loại xuất bản');
				$('#popup_modal .modal-body').html(req);
				$('#popup_modal').modal('show');
			});
		}
	...
	function active(e){
		...
		$.post('<?php echo ROOTHOST;?>ajaxs/publish_group/active.php', {"id": id}, function(res){
			if(parseInt(res)==1){
	...
	function order(e){
		...
		$.post('<?php echo ROOTHOST;?>ajaxs/publish_group/order.php', _data, function(res){
			if(parseInt(res)==1){
				window.location.reload();
			}else if(parseInt(res)==3){
				alert('Bạn không có quyền thực hiện chức năng này!');
	...
	function delete1(e){
		...
		if(confirm('Bạn có chắc muốn xóa?')){
			$.post('<?php echo ROOTHOST;?>ajaxs/publish_group/delete.php', {"id": id}, function(res){
				if(parseInt(res)==1){
					window.location.reload();
				}else if(parseInt(res)==3){
					alert('Bạn không có quyền thực hiện chức năng này!');
