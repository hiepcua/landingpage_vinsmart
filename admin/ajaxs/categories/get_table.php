...
function listTable($strwhere="", $search=0, $limit){
	...
	$res=SysGetList('tbl_categories', [], $strsql);
	if(count($res)>0){
		foreach ($res as $key => $rows) {
			...
			echo "</tr>";
			if($search !== 0){
				$res_childrent = SysGetList('tbl_categories', [], $strwhere." AND path LIKE '".$rows['path']."_%' ORDER BY `order` ASC");
				if(count($res_childrent)>0){
					foreach ($res_childrent as $key => $value) {
						...
						$par_name2 = SysGetList('tbl_categories', array('title'), "AND id=".$value['par_id']);
