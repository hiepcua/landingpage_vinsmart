	...
	<table class="table table-bordered">
		<thead>                  
			<tr>
				...
				<th>Tên chức vụ của nhân viên</th>
				<th class="text-center">Mã</th>
			</tr>
			<tbody id="model-tbl-body">
				<?php
				$res_personnel_groups = SysGetList('tbl_personnel_group', [], "ORDER BY `order` ASC, `title` ASC");
				if(count($res_personnel_groups)>0){
					foreach ($res_personnel_groups as $key => $value) {
						if(!in_array($value['id'], $ids)){
							echo '<tr>';
							echo '<td><div class="form-check">
							<input class="form-check-input ip_chk_personnel_group" data-id="'.$value['id'].'" data-name="'.$value['title'].'" name="ip_chk_personnel_group" type="checkbox">
							</div></td>';
...
<div class='form-group text-center' >
	<button class='btn btn-primary' id='ajx_add_personnel_group'><i class="fas fa-save"></i> Chọn chức vụ</button>
	<button class='btn btn-default pull-right' id='ajx_cancel_personnel_group'>Hủy <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-backspace-reverse-fill" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
...
<script type="text/javascript">
	$(document).ready(function(){
		$('#ajx_add_personnel_group').on('click', function(){
			var data=[];
			$('input:checkbox[name=ip_chk_personnel_group]:checked').each(function(){
		...
		$('#model-btn-search').on('click', (function(){
			...
			if(lg_val>2){
				var _url="<?php echo ROOTHOST;?>ajaxs/personnel/search_personnel_group.php";
		...
		$('#ajx_cancel_personnel_group').on('click', function(){
			close_model();
		});
	});
</script>