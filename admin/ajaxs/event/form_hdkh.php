...
$res_schedules = SysGetList('tbl_schedule', [], "AND event_id=".$row['id']);
?>
<div class="row">
	<div class="col-md-9">
		<div  class="form-group">
			<label>Tiêu đề<font color="red">*</font></label>
			<input type="text" id="txt_name" name="txt_name" class="form-control" value="<?php echo $row['title']; ?>" placeholder="Tiêu đề HĐKH">
		</div>
		...
		<div class="row form-group">
			<div class="col-md-5">
				...
				<input type="date" id="start_time" name="start_time" value="<?php echo $row['start_time']!==null ? date('Y-m-d', $row['start_time']) : null;?>" class="form-control">
			</div>
			<div class="col-md-5">
				<label>Thời gian kết thúc</label>
				<input type="date" id="end_time" name="end_time" class="form-control" value="<?php echo $row['end_time']!== null ? date('Y-m-d', $row['end_time']): null;?>">
			</div>
			<div class="col-md-2">
				<label>.</label>
		...
		<div class="form-group">
			...
			<textarea class="form-control" id="txt_sapo" name="txt_sapo" placeholder="Tóm tắt nội dung..." rows="3"><?php echo $row['sapo'];?></textarea>
		</div>
	...
	<div class="col-md-3">
		<div class="form-group">
			...
			<script type="text/javascript">
				$(document).ready(function(){
					cbo_Selected('cbo_cate', <?php echo $row['group_id']; ?>);
				});
			</script>
		</div>
		...
		<div  class="form-group">
			...
			<textarea id="address" name="address" class="form-control" placeholder="Địa điểm diễn ra HĐKH"><?php echo $row['address'];?></textarea>
		</div>

		<div class="form-group">
			<label>Địa điểm chi tiết</label>
			<textarea id="address2" name="address2" class="form-control" placeholder="Địa điểm chi tiết" rows="3"><?php echo $row['address2'];?></textarea>
		</div>
		...
		<div class="form-group">
			...
			<div id="list_personnel">
				<?php
				$ar_personnel_ids = $row['personnel'] !== '' && $row['personnel'] !== NULL ? json_decode($row['personnel']) : [];
			...
			<div><span class="btn btn-primary" onclick="add_personnel()"><i class="fa fa-plus-square" aria-hidden="true"></i> Thêm nhà khoa học</span></div>
		</div>
		...
		<div class="form-group">
			...
			<div id="list_research_team">
				<?php
				$ar_team_ids = $row['research_team'] !== '' && $row['research_team'] !== NULL ? json_decode($row['research_team']) : [];
		...
		<div class="form-group">
			...
			<textarea id="baocaovien" name="baocaovien" class="form-control" placeholder="Báo cáo viên ngoài viện" rows="1"><?php echo $row['baocaovien'];?></textarea>
		</div>
...
<script type="text/javascript">
	...
	function showSchedule(){
		var js_data = '<?php echo addslashes(json_encode($res_schedules));?>';
		...
		if(int_stime<=int_etime){
			$.post('<?php echo ROOTHOST;?>ajaxs/event/show_schedule.php', {'stime': stime, 'etime': etime, "data": data}, function(res){
				if(parseInt(res)!=''){
	...
	function append_event_item(data){
		$('#wg-schedule .idate').each(function(){
			...
			if(obj_data.hasOwnProperty(_date)){
				var str='<div class="event_item" onclick="edit_event_item(this)" data-id="'+obj_data[_date]['id']+'" data-date="'+obj_data[_date]['date']+'" data-sdate="'+obj_data[_date]['date']+'" data-edate="'+obj_data[_date]['date']+'" data-stime="'+obj_data[_date]['start_time']+'" data-etime="'+obj_data[_date]['end_time']+'" data-title="'+obj_data[_date]['title']+'" data-address="'+obj_data[_date]['address']+'">';
