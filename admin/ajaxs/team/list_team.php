...
<br/>
<div class="wg-header">
	<div class="form-inline"><input type="text" id="model-ip-search" name="model-ip-search" class="form-control float-sm-right"><button class="btn btn-primary" id="model-btn-search" style="margin-left: 10px;">Tìm kiếm</button></div>
</div>
<br/>
	...
	<table class="table table-bordered">
		<thead>                  
			...
			<tbody id="model-tbl-body">
				<?php
				$res_teams = SysGetList('tbl_team', [], "ORDER BY `order` ASC, `title` ASC");
...
<div class='form-group text-center' >
	<button class='btn btn-primary' id='ajx_add_team'><i class="fas fa-save"></i> Chọn nhóm nghiên cứu viên</button>
...
<script type="text/javascript">
	$(document).ready(function(){
		$('#ajx_add_team').on('click', function(){
			var data=[];
			$('input:checkbox[name=ip_chk_team]:checked').each(function(){
		...
		$('#model-btn-search').on('click', (function(){
			...
			if(lg_val>3){
				var _url="<?php echo ROOTHOST;?>ajaxs/team/search_team.php";
