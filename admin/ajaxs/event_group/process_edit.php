...
if($isAdmin!=1 && !in_array(4002, $arr_permission)){
	die('3');
}
...
if(isset($_POST['id']) && $_POST['id']!=0) {
	...
	if(isset($_FILES['txt_thumb']) && $_FILES['txt_thumb']['size'] > 0){
		$save_path 	= MEDIA_HOST."/event_group/";
	...
	$rs_parent = SysGetList('tbl_event_group', array("path"), " AND id=".$arr['par_id']);
	...
	$result = SysEdit('tbl_event_group', $arr, "id=".$_POST['id']);
	if($result){
		die('1');
	}else{
		die('0');
