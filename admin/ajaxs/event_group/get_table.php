...
function listTable($strwhere="", $search=0, $limit){
	...
	$res=SysGetList('tbl_event_group', [], $strsql);
	if(count($res)>0){
		foreach ($res as $key => $rows) {
			...
			$par_name = SysGetList('tbl_event_group', array('title'), "AND id=".$rows['par_id']);
			...
			echo "</tr>";
			if($search !== 0){
				$res_childrent = SysGetList('tbl_event_group', [], "AND path LIKE '".$rows['path']."_%' ORDER BY `order` ASC");
				if(count($res_childrent)>0){
					foreach ($res_childrent as $key => $value) {
						...
						$par_name2 = SysGetList('tbl_event_group', array('title'), "AND id=".$value['par_id']);
						...
						$link2 = ROOTHOST_WEB.'hoat-dong-khoa-hoc/'.$value['alias'];
						...
						$order = $value['order'];
						echo "<td width='50' class='text-center'><input style='width: 50px;' type='number' min='0' name='txt_order' value=\"$order\" size=\"4\" class=\"order text-center\" onchange='order(this)' data-id='".$value['id']."'></td>";

						echo "<td class='text-center'><a href='#' onclick='active(this)' data-id='".$value['id']."'>".$icon_active."</a></td>";
