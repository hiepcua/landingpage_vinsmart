	...
	<div class="row">
		<div class="col-md-8">
			<div class="form-group">
				...
				<input type="text" id="txt_name" name="txt_name" class="form-control" value="<?php echo $row['title'];?>" placeholder="Tiêu đề danh mục hoạt động khoa học">
			</div>
			...
			<div class="form-group">
				<label>Mô tả</label>
				<textarea class="form-control" name="txt_intro" placeholder="Mô tả về danh mục hoạt động khoa học..." rows="2"><?php echo $row['intro'];?></textarea>
			</div>
		</div>
	...
	$(document).ready(function(){
		$("form#frm_action").submit(function(e) {
			if(validForm()){
				...
				var _url="<?php echo ROOTHOST;?>ajaxs/event_group/process_edit.php";
				$.ajax({
					url: _url,
					type: 'POST',
					data: formData,
