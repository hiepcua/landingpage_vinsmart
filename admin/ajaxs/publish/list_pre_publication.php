	...
	<table class="table table-bordered">
		<thead>                  
			...
			<tbody id="model-tbl-body">
				<?php
				$res_pre_publications = SysGetList('tbl_publish', [], "AND `group_code`='tienanpham' ORDER BY `order` ASC, `title` ASC");
...
<div class='form-group text-center' >
	<button class='btn btn-primary' id='ajx_add_pre_publication'><i class="fas fa-save"></i> Chọn tiền ấn phẩm</button>
...
<script type="text/javascript">
	$(document).ready(function(){
		$('#ajx_add_pre_publication').on('click', function(){
			var data=[];
			$('input:checkbox[name=ip_chk_publish]:checked').each(function(){
		...
		$('#model-btn-search').on('click', (function(){
			...
			if(lg_val>2){
				var _url="<?php echo ROOTHOST;?>ajaxs/publish/search_publish.php";
				$.post(_url, {'key': val, 'ids': ids, 'group_code': 'tienanpham'}, function(req){
					$('#model-tbl-body').html(req);
				});
			}else{
				var _url="<?php echo ROOTHOST;?>ajaxs/publish/search_publish.php";
				$.post(_url, {"ids": ids, 'group_code': 'tienanpham'}, function(req){
					$('#model-tbl-body').html(req);
				});
			}
		}));
